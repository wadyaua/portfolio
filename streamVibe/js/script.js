...
function initSpoilers() {
	...
	spoilers.forEach(spoiler => {
		...
		summary.addEventListener('click', async (e) => {
			...
			// ✅ FIX: reset flag after animation fully completes, not after arbitrary 10ms
			isAnimating = false;
		});
	});
}
...
async function initPriceSwitcher() {
	...
	document.addEventListener('click', (e) => {
		...
		// ✅ FIX: guard against null (e.g. switcher not on current page)
		...
		priceBlocks.forEach((block, i) => {
			if (!prices[i]) return; // guard against index mismatch
...
function initCollectionSwitcher() {
	// Clean up any previous mq listener that holds stale DOM refs
...
function initModals() {
	...
	function safeOpen(modal) {
		if (modal.hasAttribute('open')) return; // already open
		modal.showModal();
	...
	function safeClose(modal) {
		...
		if (modal.dataset.closing) return; // ✅ FIX: prevent double-close
		modal.dataset.closing = '1';
		modal.classList.remove('is-open');

		setTimeout(() => {
			delete modal.dataset.closing;
			// Guard: dialog may have been force-closed by SWUP in the meantime
...
// ============================================================
/**
 * Global listeners — attached to `document` once for the lifetime
 * of the page. NEVER re-called on SWUP transitions.
 */
function initGlobalListeners() {
	// Burger — single delegated listener on document
	document.addEventListener('click', (e) => {
...
/**
 * Page components — must re-run after every SWUP content replace
 * because they query the DOM for elements that get swapped out.
 */
...
swup.hooks.on('visit:start', () => {
	closeBurgerMenu();
	destroySwipers(); // ✅ destroy before DOM is replaced

	// Force-close all open dialogs immediately (no animation, no race)
	document.querySelectorAll('dialog[open]').forEach(dialog => {
		dialog.classList.remove('is-open');
		delete dialog.dataset.closing;
		dialog.close(); // triggers 'close' event → ScrollLock.unlock
	});

	ScrollLock.forceUnlock(); // nuclear fallback
});
